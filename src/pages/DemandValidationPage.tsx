...
export default function DemandValidationPage() {
  ...
  const validCount = requests.filter((request) => typeof request.ai_status === "string" && request.ai_status === "valid").length;
      ...
      {!listQuery.isLoading && !listQuery.isError && (
        <div className="grid gap-4">
          {requests.map((request) => {
            const aiStatus = request.ai_status ?? "pending";
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="space-y-1 mt-1 text-xs">
                        ...
                        <p>
                          <span className="text-muted-foreground">Budget feedback:</span> {budgetFeedback}
                        </p>
                      </div>
                    </div>
                  </div>
                  <StatusBadge status={aiStatus} className="min-w-[110px] justify-center" />
                </div>

                <div className="mt-3 text-xs text-muted-foreground">
                  AI decision: {getAiStatusLabel(aiStatus)}
                </div>
              </div>
            );
          })}
